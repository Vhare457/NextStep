...
namespace Job_Application.Repositories
{
    public class JobPostRepository : IJobPostRepository
    {
        ...
        {
            // Load all related job applications with their dependencies
            await _context.Entry(job)
                .Collection(jp => jp.JobApplications)
                .Query()
                .Include(ja => ja.InterviewSchedule)
                .LoadAsync();

            // Get all job application IDs for this job post
            var jobApplicationIds = job.JobApplications.Select(ja => ja.JobApplicationId).ToList();

            if (jobApplicationIds.Any())
            {
                // Delete interview schedules associated with these applications
                var interviewSchedules = await _context.InterviewSchedules
                    .Where(i => jobApplicationIds.Contains(i.JobApplicationId))
                    .ToListAsync();
                
                if (interviewSchedules.Any())
                {
                    _context.InterviewSchedules.RemoveRange(interviewSchedules);
                }
